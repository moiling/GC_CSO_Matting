clear
path = '/home/gdsb/SCUT/YH/Image Matting/data/train/low_res/';
img_path = [path,'img/'];
trimap_name = 'Trimap1/';
...
parpool('local', 20);
parfor i =11:30
    output_path = sprintf('3e5_%d/%s',i,trimap_name);
    if ~exist(output_path,'dir')
        mkdir(output_path);
    end
    for m = 11:length(img_dir)
        disp(img_dir(m).name);
        ...
        filename = sprintf('%s%s.mat',output_path,img_dir(m).name(1:end-4));
