...
MyFuzzyCostFunc( x,F_rgb,B_rgb,U_rgb,F_s,B_s,U_s,F_mindist,B_mindist)
...
%   Detailed explanation goes here
    ...
    cc_mem = exp(-0.11*cost_c);
    dfc_mem = exp(-0.17*(cost_sF - 1));
    dbc_mem = exp(-0.17*(cost_sB - 1));
    dc_mem = 0.5*(dfc_mem + dbc_mem);
    fitness = -(cc_mem.*dc_mem) ./ (1 - (1 - cc_mem).*(1 - dc_mem));
end
