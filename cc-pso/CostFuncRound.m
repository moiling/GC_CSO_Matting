...
CostFuncRound( x,F_rgb,B_rgb,U_rgb,F_s,B_s,U_s,F_mindist,B_mindist)
...
%   Detailed explanation goes here
    ...
    est_alphaX3 = repmat(est_alpha,1,3);
    cost_c  = norm2(U_rgb-(est_alphaX3.*Fx_rgb+(1-est_alphaX3).*Bx_rgb));
    % Spatial cost
    cost_sF = norm2(Fx_s-U_s)./F_mindist;
    cost_sB = norm2(Bx_s-U_s)./B_mindist;
    cost_cd = norm2(Fx_Bx_rgb);
    fitness = (cost_c+cost_sF+cost_sB);
end
